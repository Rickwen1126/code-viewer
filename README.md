...
└─ CodeTour                            (no business logic)          └─ Cache-first offline
```
...
- Line numbers (Shiki transformer `data-line`)
...
- Zombie WS detection — 檢查 `readyState`，強制 reconnect
- Cache-first loading — 重連期間顯示本地快取資料
- Error Boundary — 萬一 React crash，顯示 reload 按鈕
...
### [`002-mobile-viewer`](../../tree/002-mobile-viewer) → `main` — 當前架構

- **三層架構**：VS Code Extension → Hono Relay Backend → React PWA
- 75 functional requirements, 69 tasks, 166 unit tests
