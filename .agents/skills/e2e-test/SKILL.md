...
- Default for code-viewer E2E: use `/codeview-dev`, which launches `tests/e2e/launch-extension.mjs --real`
- `--real` means installed VS Code binary + `~/.vscode/extensions`
- `--real` does **not** mean your full daily VS Code profile/session; it does not load `~/Library/Application Support/Code`
- Only `--copilot` mode uses your normal user-data-dir, and that mode requires your own VS Code to be closed
- Use lightweight mode only for intentional pure-frontend smoke checks; if you do, report that extension/LSP/round-trip coverage is degraded

### Freshness check

Before running E2E, confirm the app under test is actually the latest build:

1. Check whether `extension/src/**/*.ts` is newer than `extension/dist/extension.js`
2. If yes, rebuild the extension before launching the extension host
...
| 26a | Tour detail: Edit step preserves title | Edit a step that already has a title | Modify content → Save. **Verify: original `step.title` still renders after save and after round-trip re-open. This was a real bug — edit flow deleted and re-added the step without `title`.** |
